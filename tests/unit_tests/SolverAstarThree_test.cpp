...
TEST_CASE("goal_board == start_board (1-step solution)") {
  ...
  SolverInformed<EstGoalCostAstarThree> solver{ boards.at(0), boards.at(num_steps - 1), move_cost };
  size_t i{0};
...
TEST_CASE("6-step \"easy\" solution") {
  ...
  constexpr size_t num_fq_nodes_popped{ 20 };
  constexpr size_t max_fq_size{ 12 };
  std::array<Board, num_steps> boards{
...
TEST_CASE("\"medium\" solution") {
  ...
  constexpr size_t num_fq_nodes_popped{ 94 };
  constexpr size_t max_fq_size{ 55 };
  std::array<Board, 2> boards{ Board{"281043765"}, Board{"123804765"} };
  auto move_cost{ std::make_shared<MoveCostSqVal>() };
  SolverInformed<EstGoalCostAstarThree> solver{ boards.at(0), boards.at(1), move_cost };
  auto solution{ solver.solve() };
...
// passes, but takes about 26 sec to run
TEST_CASE("\"hard\" solution" * doctest::skip(true)) {

  constexpr size_t num_steps{ 31 };
  constexpr size_t path_cost{ 128 };
  constexpr size_t num_fq_nodes_popped{ 177376 };
  constexpr size_t max_fq_size{ 21182 };
