...
*******************************************************************************/
  ...
  C->add_child(E);
  D->add_child(F);
  EstGoalCostAstarTwo est{ goal_board };

  SUBCASE("A estimated cost") {
    CHECK_EQ(est.get_est_goal_cost(A), 14);
  }

  SUBCASE("B estimated cost") {
    CHECK_EQ(est.get_est_goal_cost(B), 13);
  }

  SUBCASE("C estimated cost") {
    CHECK_EQ(est.get_est_goal_cost(C), 15);
  }

  SUBCASE("D estimated cost") {
    CHECK_EQ(est.get_est_goal_cost(D), 19);
  }

  SUBCASE("E estimated cost") {
    CHECK_EQ(est.get_est_goal_cost(E), 23);
  }

  SUBCASE("F estimated cost") {
    CHECK_EQ(est.get_est_goal_cost(F), 22);
  }
