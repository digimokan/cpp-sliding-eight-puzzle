...
TEST_CASE("goal_board == start_board (1-step solution)") {
  ...
  SolverInformed<EstGoalCostAstarTwo> solver{ boards.at(0), boards.at(num_steps - 1), move_cost };
  size_t i{0};
...
TEST_CASE("6-step \"easy\" solution") {
  ...
  constexpr size_t num_fq_nodes_popped{ 10 };
...
TEST_CASE("\"medium\" solution") {
  ...
  constexpr size_t num_fq_nodes_popped{ 46 };
  constexpr size_t max_fq_size{ 36 };
  std::array<Board, 2> boards{ Board{"281043765"}, Board{"123804765"} };
  auto move_cost{ std::make_shared<MoveCostSqVal>() };
  SolverInformed<EstGoalCostAstarTwo> solver{ boards.at(0), boards.at(1), move_cost };
  auto solution{ solver.solve() };
...
TEST_CASE("\"hard\" solution" * doctest::skip(true)) {
  ...
  constexpr size_t num_fq_nodes_popped{ 168403 };
  constexpr size_t max_fq_size{ 25549 };
