...
TEST_CASE("goal_board == start_board (1-step solution)") {
  ...
  SolverLimitedDFS solver{ boards.at(0), boards.at(num_steps - 1), move_cost, num_steps};
  size_t i{0};
...
TEST_CASE("2-step solution" * doctest::skip(true)) {
  ...
  constexpr size_t max_fq_size{ 3 };
  std::array<Board, num_steps> boards{
  ...
  auto move_cost{ std::make_shared<MoveCostSqVal>() };
  SolverLimitedDFS solver{ boards.at(0), boards.at(num_steps - 1), move_cost, num_steps};
  SolverLimitedDFS short_solver{ boards.at(0), boards.at(num_steps - 1), move_cost, short_depth};
  size_t i{0};
  auto act = [&boards, &move_dirs, &move_costs, &i] (auto node) {
    CHECK_UNARY(node->get_board() == boards.at(i));
    CHECK_UNARY(node->get_move_dir() == move_dirs.at(i));
