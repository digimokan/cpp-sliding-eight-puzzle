...
TEST_CASE("goal_board == start_board (1-step solution)") {
  ...
  SolverInformed<EstGoalCostAstarOne> solver{ boards.at(0), boards.at(num_steps - 1), move_cost};
  size_t i{0};
...
TEST_CASE("3-step solution") {
  ...
  constexpr size_t num_fq_nodes_popped{ 5 };
...
TEST_CASE("6-step \"easy\" solution") {
  ...
  constexpr size_t num_fq_nodes_popped{ 13 };
...
TEST_CASE("\"medium\" solution") {
  ...
  constexpr size_t num_fq_nodes_popped{ 64 };
  constexpr size_t max_fq_size{ 49 };
  std::array<Board, 2> boards{ Board{"281043765"}, Board{"123804765"} };
  auto move_cost{ std::make_shared<MoveCostSqVal>() };
  SolverInformed<EstGoalCostAstarOne> solver{ boards.at(0), boards.at(1), move_cost};
  auto solution{ solver.solve() };
...
// passes, but takes about 11 sec to run
TEST_CASE("\"hard\" solution" * doctest::skip(true)) {

  constexpr size_t num_steps{ 31 };
  constexpr size_t path_cost{ 128 };
  constexpr size_t num_fq_nodes_popped{ 178009 };
  constexpr size_t max_fq_size{ 23117 };
