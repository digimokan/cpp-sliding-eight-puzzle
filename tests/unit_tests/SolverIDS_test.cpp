...
TEST_CASE("goal_board == start_board (1-step solution)") {
  ...
  SolverIDS solver{ Board{"123804765"}, Board{"123804765"}, std::make_shared<MoveCostSqVal>()};
...
TEST_CASE("3-step solution") {
  ...
  SUBCASE("get_time_complexity()") {
    SolverIDS solver{ BA, BC, std::make_shared<MoveCostSqVal>()};
    auto solution{ solver.solve() };
    CHECK_EQ(solution.get_time_complexity(), 7);
  }

  SUBCASE("get_space_complexity()") {
    SolverIDS solver{ BA, BC, std::make_shared<MoveCostSqVal>()};
    auto solution{ solver.solve() };
    CHECK_EQ(solution.get_space_complexity(), 6);
  }
...
TEST_CASE("6-step \"easy\" solution") {
  ...
  SUBCASE("get_time_complexity()") {
    SolverIDS solver{ BA, BF, std::make_shared<MoveCostSqVal>()};
    auto solution{ solver.solve() };
    CHECK_EQ(solution.get_time_complexity(), 63);
  }
